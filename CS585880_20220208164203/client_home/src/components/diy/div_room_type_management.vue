<template>
	<div class="diy_details_box diy_div_room_type_management">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "room_type_management_id",
					source_id: this.obj["room_type_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/room_type_management/set?room_type_management_id=' + _this.obj["room_type_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){
