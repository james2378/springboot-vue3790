<template>
	<div class="diy_details page_hotel_management" id="hotel_management_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_hotel_management'>
							<div_hotel_management :obj="obj"></div_hotel_management>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_hotel_management from "@/components/diy/div_hotel_management.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "hotel_management_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/hotel_management/set?hotel_management_id=' + obj["hotel_management_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "hotel_management",
							source_field: "hotel_management_id",
							source_id: this.obj.hotel_management_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.hotel_management_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "hotel_management",
					source_field: "hotel_management_id",
					source_id: obj["hotel_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/hotel_management/set?hotel_management_id=' + _this.obj["hotel_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){
